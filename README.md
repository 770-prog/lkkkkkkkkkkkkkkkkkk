...
<head>
  ...
  <title>EducatedTalk - 에듀케이티드톡</title>
  <!-- Tailwind CSS CDN -->
...
<body class="bg-neutral-100 flex items-center justify-center min-h-screen p-0 sm:p-6 text-neutral-800">

  <!-- 모바일 화면 규격의 디바이스 프레임 컨테이너 -->
  <div class="w-full h-screen sm:h-[840px] sm:max-w-[420px] bg-white sm:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col relative border-4 border-neutral-900 sm:border-[10px]">
  ...
  <script>
    // UUID 대용 고유 식별자 생성 유틸리티
    const generateId = () => Math.random().toString(36).substring(2, 15);

    // --- State Management ---
    let currentUser = null;
    let currentView = 'login'; // login, main-followers, main-educhat, main-settings
    let friendList = [];       // 가짜 데이터 일절 없이 0명으로 시작
    let chatRooms = [];        // 0개 대화방으로 시작
    let messages = {};         // roomId: [message, ...] 구조의 메시지 매핑
    let activeRoomId = null;

    // --- DOM Elements ---
    const viewLogin = document.getElementById('view-login');
    const viewMain = document.getElementById('view-main');
    const viewChatRoom = document.getElementById('view-chat-room');
    
    // Header
    const mainHeader = document.getElementById('main-header');
    const headerTitle = document.getElementById('header-title');
    const btnAddFollower = document.getElementById('btn-add-follower');
    ...
    const addFollowerForm = document.getElementById('add-follower-form');
    ...
    const addFollowerModal = document.getElementById('add-follower-modal');

    const messageForm = document.getElementById('message-form');
    const messageInput = document.getElementById('message-input');

    // Subviews
    const subviewFollowers = document.getElementById('subview-followers');
    const subviewEduchat = document.getElementById('subview-educhat');
    ...
    const chatsCountSpan = document.getElementById('chats-count');
    ...
    const tabEduchat = document.getElementById('tab-educhat');
    ...
    function setView(view) {
      ...
      // Reset active states
      subviewFollowers.classList.add('hidden');
      subviewEduchat.classList.add('hidden');
      subviewSettings.classList.add('hidden');
      btnAddFollower.classList.add('hidden');

      tabFollowers.className = "flex flex-col items-center gap-1.5 transition-all text-neutral-400 hover:text-neutral-600";
      tabEduchat.className = "flex flex-col items-center gap-1.5 transition-all text-neutral-400 hover:text-neutral-600 relative";
      tabSettings.className = "flex flex-col items-center gap-1.5 transition-all text-neutral-400 hover:text-neutral-600";
      ...
      if (view === 'login') {
        ...
        if (view === 'main-followers') {
          subviewFollowers.classList.remove('hidden');
          headerTitle.innerText = "팔로워";
          btnAddFollower.classList.remove('hidden');
          tabFollowers.className = "flex flex-col items-center gap-1.5 transition-all text-neutral-950 scale-105";
          renderFollowers();
        } else if (view === 'main-educhat') {
          subviewEduchat.classList.remove('hidden');
          headerTitle.innerText = "EduChat";
          tabEduchat.className = "flex flex-col items-center gap-1.5 transition-all text-neutral-950 scale-105 relative";
          renderChatRooms();
        } else if (view === 'main-settings') {
          subviewSettings.classList.remove('hidden');
          headerTitle.innerText = "환경 설정";
          tabSettings.className = "flex flex-col items-center gap-1.5 transition-all text-neutral-950 scale-105";
          renderSettings();
        }
      }
    }
    ...
    loginForm.addEventListener('submit', (e) => {
      ...
      if (!input) {
        showLoginError('이메일 주소 또는 전화번호를 입력해주세요.');
        return;
      }

      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      const phoneRegex = /^010[- ]?\d{3,4}[- ]?\d{4}$|^\d{9,11}$/;

      const isEmail = emailRegex.test(input);
      const isPhone = phoneRegex.test(input);

      if (!isEmail && !isPhone) {
        showLoginError('올바른 이메일 형식 또는 전화번호 형식이 아닙니다.');
        return;
      }

      hideLoginError();

      // [51-100] User Profile 스키마 기반 데이터 생성
      const usernameFromInput = isEmail ? input.split('@')[0] : `user_${input.slice(-4)}`;
      currentUser = {
        user_id: generateId(),
        username: usernameFromInput,
        display_name: usernameFromInput,
        email_address: isEmail ? input : null,
    ...
    btnAddFollower.addEventListener('click', () => {
      followerSearchInput.value = '';
      followerError.classList.add('hidden');
      addFollowerModal.classList.remove('hidden');
      followerSearchInput.focus();
    });

    btnCloseModal.addEventListener('click', () => {
      addFollowerModal.classList.add('hidden');
    });

    addFollowerForm.addEventListener('submit', (e) => {
      e.preventDefault();
      const targetName = followerSearchInput.value.trim();

      if (!targetName) {
        showFollowerError('추가할 사용자 닉네임을 입력해주세요.');
        return;
      }

      if (friendList.some(f => f.display_name === targetName)) {
        showFollowerError('이미 팔로우 목록에 존재하는 사용자입니다.');
        return;
      }
      ...
      const newFollower = {
        ...
        status_message: '에듀케이티드톡 대기 중'
      };

      friendList.push(newFollower);
      addFollowerModal.classList.add('hidden');
      renderFollowers();
    });

    function showFollowerError(msg) {
      followerError.innerText = msg;
      followerError.classList.remove('hidden');
    }

    function renderFollowers() {
      if (friendList.length === 0) {
        followersEmpty.classList.remove('hidden');
        followersListContainer.classList.add('hidden');
      } else {
        followersEmpty.classList.add('hidden');
        followersListContainer.classList.remove('hidden');
        followersCountSpan.innerText = friendList.length;

        followersList.innerHTML = '';
        friendList.forEach(friend => {
          const li = document.createElement('li');
          li.className = "flex items-center justify-between p-4 bg-white rounded-2xl border border-neutral-100 shadow-sm";
          
          li.innerHTML = `
            <div class="flex items-center gap-3">
              <div class="w-11 h-11 bg-neutral-900 text-white rounded-xl flex items-center justify-center font-black text-sm relative select-none">
                ${friend.display_name.charAt(0).toUpperCase()}
                ${friend.is_online ? '<div class="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-500 border-2 border-white rounded-full"></div>' : ''}
              </div>
              <div>
                <h4 class="font-bold text-neutral-900">${friend.display_name}</h4>
                <p class="text-xs text-neutral-400">${friend.status_message}</p>
              </div>
            </div>
            <button 
              onclick="openChatRoom('${friend.user_id}')"
              class="px-3.5 py-1.5 bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-bold rounded-lg transition active:scale-95"
            >
              대화
            </button>
          `;
    ...
    // --- Chat Room Logic ---
    window.openChatRoom = function(targetUserId) {
      const targetUser = friendList.find(f => f.user_id === targetUserId);
      if (!targetUser) return;

      // 1:1 관계 대화방 탐색
      let room = chatRooms.find(r => r.room_type === '1:1' && r.members.includes(currentUser.user_id) && r.members.includes(targetUserId));

      if (!room) {
        room = {
          room_id: generateId(),
          room_type: '1:1',
          room_title: `${targetUser.display_name}님과의 대화`,
          creator_id: currentUser.user_id,
          created_at: Date.now(),
          members: [currentUser.user_id, targetUserId],
          target_user: targetUser,
          last_message_text: '새로운 대화가 시작되었습니다.',
      ...
      // 채팅방 내용 채우기
      chatRoomTitle.innerText = targetUser.display_name;
      chatRoomAvatar.innerText = targetUser.display_name.charAt(0).toUpperCase();
    ...
    messageForm.addEventListener('submit', (e) => {
      ...
      messages[activeRoomId].push(newMessage);

      // 방 최신 메시지 기록 업데이트
      const room = chatRooms.find(r => r.room_id === activeRoomId);
      if (room) {
        room.last_message_text = content;
        room.last_message_at = Date.now();
      }

      // 대화방 순서 정렬 (최신 대화순)
      chatRooms.sort((a, b) => b.last_message_at - a.last_message_at);
    ...
    function renderMessages() {
      ...
      const list = messages[activeRoomId] || [];
      ...
      list.forEach(msg => {
        ...
        div.innerHTML = `
          ${!isMine ? '<span class="text-[10px] text-neutral-400 select-none pb-1">수신됨</span>' : ''}
          <div class="max-w-[70%] flex flex-col ${isMine ? 'items-end' : 'items-start'}">
            <div class="px-4 py-2.5 text-sm leading-relaxed shadow-sm ${isMine ? 'bg-neutral-900 text-white rounded-2xl rounded-tr-none' : 'bg-white text-neutral-800 border border-neutral-100 rounded-2xl rounded-tl-none'}">
              ${escapeHtml(msg.content)}
            </div>
            <span class="text-[9px] text-neutral-400 mt-1 px-1">${timeString}</span>
          </div>
          ${isMine ? `
            <span class="text-[9px] text-neutral-400 select-none pb-1 flex items-center gap-0.5">
              읽음 
    ...
    function renderChatRooms() {
      if (chatRooms.length === 0) {
        ...
        chatsCountSpan.innerText = chatRooms.length;

        chatsList.innerHTML = '';
        chatRooms.forEach(room => {
          const li = document.createElement('li');
          li.className = "flex items-center gap-3.5 p-4 bg-white rounded-2xl border border-neutral-100 shadow-sm cursor-pointer hover:bg-neutral-50/50 transition active:scale-[0.99]";
          
          const timeString = new Date(room.last_message_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

          li.innerHTML = `
            <div class="w-12 h-12 bg-neutral-100 rounded-xl flex items-center justify-center font-black text-neutral-700 text-lg border border-neutral-200 select-none">
              ${room.target_user.display_name.charAt(0).toUpperCase()}
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex justify-between items-baseline mb-0.5">
                <h4 class="font-bold text-neutral-900 truncate">${room.target_user.display_name}</h4>
                <span class="text-[10px] text-neutral-400">${timeString}</span>
              </div>
              <p class="text-xs text-neutral-500 truncate">${escapeHtml(room.last_message_text)}</p>
            </div>
          `;
          
          li.addEventListener('click', () => {
            openChatRoom(room.target_user.user_id);
          });
    ...
    function renderSettings() {
      ...
      document.getElementById('settings-avatar').innerText = currentUser.display_name.charAt(0).toUpperCase();
      document.getElementById('settings-username').innerText = currentUser.display_name;
      document.getElementById('settings-contact').innerText = currentUser.email_address || currentUser.phone_number;
      document.getElementById('settings-method').innerText = currentUser.email_address ? '이메일 인증' : '전화번호 인증';
    }
    ...
    tabFollowers.addEventListener('click', () => setView('main-followers'));
    tabEduchat.addEventListener('click', () => setView('main-educhat'));
    tabSettings.addEventListener('click', () => setView('main-settings'));
